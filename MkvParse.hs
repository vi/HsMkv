...
demoHandler (ME_Tracks tracks) = mapM_ ttt tracks
    where
    ttt :: Track -> IO()
    ttt t = do
        printf "Track %d\n"                    $ t_number t
        printf "  Type: %s\n"      $ show      $ t_type t
        printf "  CodecID: %s\n"   $ T.unpack  $ t_codecId t
        printf "  Language: %s\n"  $ T.unpack  $ t_language t

demoHandler (ME_Frame frame) = do
    printf "Frame for %d ts=%.06f lace=%d %s len=%d data=%s...\n" tn ts lace dur len bufstr
        where
        tn = f_trackNumber frame
        ts = f_timeCode frame
        buf = B.concat $ f_data frame
        bufstr =  T.unpack $ M.toHex $ B.take 10 buf
        lace = length $ f_data frame
        len = B.length $ B.concat $ f_data frame
        dur = case f_duration frame of
            Nothing -> ""
